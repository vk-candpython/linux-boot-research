...
**What it does:** Executes a system command and captures its stdout as a string — used for parsing findmnt, lsblk, and mountvol output for device discovery. Returns empty string on any failure.
...
**What it finds:** Recursively walks the ESP mount point and collects all .efi file paths — these are the UEFI boot applications (bootx64.efi, grubx64.efi, systemd-bootx64.efi, etc.) that the firmware loads. Returns the list for subsequent overwriting.
